...
int sensor_imx682_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	ret = is_sensor_write8(client, SENSOR_IMX682_OTP_PAGE_SETUP_ADDR, 0x7F);

	/* Turn ON OTP Read MODE */
	ret |= is_sensor_write8(client, SENSOR_IMX682_OTP_READ_TRANSFER_MODE_ADDR,  0x01);
	if (ret < 0) {
	...
	ret = is_sensor_read8(client, SENSOR_IMX682_OTP_STATUS_REGISTER_ADDR,  &status);
	if ((status & 0x1) == false)
...
***********************************************************************/
	...
	memset(cis->cis_data, 0, sizeof(cis_shared_data));
	ret = sensor_imx682_cis_check_rev_on_init(subdev);
	if (ret < 0) {
		warn("Checking imx682 rev is failed during cis init, ret(%d)", ret);
		ret = -EINVAL;
		goto p_err;
	}
#endif	
...
static struct is_cis_ops cis_ops_imx682 = {
	...
	.cis_check_rev_on_init = sensor_imx682_cis_check_rev_on_init,
};
