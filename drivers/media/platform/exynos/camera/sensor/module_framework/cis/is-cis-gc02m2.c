...
#include "is-cis-gc02m2-setA.h"
#include "is-cis-gc02m2-setB.h"
...
static const u32 *sensor_gc02m2_global;
static u32 sensor_gc02m2_global_size;
static const u32 **sensor_gc02m2_setfiles;
static const u32 *sensor_gc02m2_setfile_sizes;
static const struct sensor_pll_info_compact **sensor_gc02m2_pllinfos;
static u32 sensor_gc02m2_max_setfile_num;
static const u32 *sensor_gc02m2_fsync_master;
static u32 sensor_gc02m2_fsync_master_size;
static const u32 *sensor_gc02m2_fsync_slave;
static u32 sensor_gc02m2_fsync_slave_size;

static bool sensor_gc02m2_check_master_stream_off(struct is_core *core)
{
...
static void sensor_gc02m2_cis_data_calculation(const struct sensor_pll_info_compact *pll_info, cis_shared_data *cis_data)
{
	u32 vt_pix_clk_hz = 0;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_GC02M2_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_GC02M2_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_GC02M2_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_GC02M2_COARSE_INTEGRATION_TIME_MAX_MARGIN;
	info("%s: done", __func__);
}

static int sensor_gc02m2_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
int sensor_gc02m2_cis_check_rev(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u8 rev = 0;
	...
	probe_info("gc02m2 cis_check_rev start\n");

	I2C_MUTEX_LOCK(cis->i2c_lock);

	/* OTP Read initial setting */
	ret = sensor_cis_set_registers_addr8(subdev, sensor_gc02m2_otp_read, ARRAY_SIZE(sensor_gc02m2_otp_read));
	if (ret < 0) {
		err("sensor_gc02m2_set_registers fail!!");
		goto p_err;
	}
...
u32 sensor_gc02m2_cis_calc_again_closest(u32 permile)
{
	int i;

	if (permile <= sensor_gc02m2_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_gc02m2_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX];
	if (permile >= sensor_gc02m2_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_gc02m2_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX];

	for (i = 0; i <= MAX_GAIN_INDEX; i++)
	{
		if (sensor_gc02m2_analog_gain[i][PERMILE_GAIN_INDEX] == permile)
			return sensor_gc02m2_analog_gain[i][PERMILE_GAIN_INDEX];

		if ((int)(permile - sensor_gc02m2_analog_gain[i][PERMILE_GAIN_INDEX]) < 0)
			return sensor_gc02m2_analog_gain[i-1][PERMILE_GAIN_INDEX];
	}
...
u32 sensor_gc02m2_cis_calc_again_permile(u8 code)
{
	u32 ret = 0;
	int i;

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_gc02m2_analog_gain[i][0] == code) {
			ret = sensor_gc02m2_analog_gain[i][1];
			break;
		}
	}
...
u32 sensor_gc02m2_cis_calc_again_code(u32 permile)
{
	u32 ret = 0, nearest_val = 0;
	int i;

	dbg_sensor(2, "[%s] permile %d\n", __func__, permile);
	nearest_val = sensor_gc02m2_cis_calc_again_closest(permile);
	dbg_sensor(2, "[%s] nearest_val %d\n", __func__, nearest_val);

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_gc02m2_analog_gain[i][1] == nearest_val) {
			ret = sensor_gc02m2_analog_gain[i][0];
			break;
		}
	}
...
int sensor_gc02m2_cis_set_exposure_time(struct v4l2_subdev *subdev, u16 multiple_exp)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
u32 sensor_gc02m2_cis_calc_dgain_code(u32 input_gain, u32 permile)
{
	u32 calc_value = 0;
	u32 digital_gain = 0;
...
int sensor_gc02m2_cis_set_analog_gain(struct v4l2_subdev *subdev, u32 input_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	analog_gain = sensor_gc02m2_cis_calc_again_code(input_again );
	analog_permile = sensor_gc02m2_cis_calc_again_permile(analog_gain);
	digital_gain = sensor_gc02m2_cis_calc_dgain_code(input_again, analog_permile);
...
int sensor_gc02m2_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	return 0;
}
#else
...
/* CIS OPS */
int sensor_gc02m2_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_width = SENSOR_GC02M2_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_GC02M2_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_gc02m2_cis_data_calculation(sensor_gc02m2_pllinfos[setfile_index], cis->cis_data);
...
int sensor_gc02m2_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client = NULL;
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	sensor_cis_dump_registers(subdev, sensor_gc02m2_setfiles[0], sensor_gc02m2_setfile_sizes[0]);
...
int sensor_gc02m2_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_cis_set_registers_addr8(subdev, sensor_gc02m2_global, sensor_gc02m2_global_size);
	if (ret < 0) {
		err("sensor_gc02m2_set_registers fail!!");
		goto p_err;
	}
...
int sensor_gc02m2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	struct i2c_client *client = NULL;
	...
	if (mode > sensor_gc02m2_max_setfile_num) {
	...
	if (cis->rev_flag == true) {
		...
		if (ret < 0) {
			err("sensor_gc02m2_check_rev is fail");
			goto p_err;
		}
	}

	sensor_gc02m2_cis_data_calculation(sensor_gc02m2_pllinfos[mode], cis->cis_data);

	/* This delay restrains critical issues. If entry time issue comes up, this delay should be removed */
	msleep(50);

	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_cis_set_registers_addr8(subdev, sensor_gc02m2_setfiles[mode], sensor_gc02m2_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_gc02m2_set_registers fail!!");
		goto p_i2c_err;
	}
...
int sensor_gc02m2_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_gc02m2_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_GC02M2_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_GC02M2_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_GC02M2_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_GC02M2_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_GC02M2_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_GC02M2_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_gc02m2_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
//#endif
	...
	if (single_mode) {
		...
		ret = sensor_cis_set_registers_addr8(subdev, sensor_gc02m2_fsync_slave, sensor_gc02m2_fsync_slave_size);
		if (ret < 0)
			err("[%s] sensor_gc02m2_fsync_slave fail\n", __func__);
	...
	info("[gc02m2] StreamOn setting done.\n");
	if (IS_ENABLED(DEBUG_SENSOR_TIME))
		dbg_sensor(1, "[%s] time %ldus", __func__, PABLO_KTIME_US_DELTA_NOW(st));
...
int sensor_gc02m2_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	info("[gc02m2] StreamOff setting done \n");
...
int sensor_gc02m2_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	cis_shared_data *cis_data = NULL;
...
int sensor_gc02m2_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gc02m2_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_gc02m2_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (sensor_gc02m2_check_master_stream_off(core)) {
...
int sensor_gc02m2_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_gc02m2_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_gc02m2_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gc02m2_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	*again = sensor_gc02m2_cis_calc_again_permile(analog_gain);
...
int sensor_gc02m2_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = SENSOR_GC02M2_MIN_ANALOG_GAIN_SET_VALUE;

	cis_data->min_analog_gain[0] = read_value;

	cis_data->min_analog_gain[1] = sensor_gc02m2_cis_calc_again_permile(read_value);
...
int sensor_gc02m2_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = SENSOR_GC02M2_MAX_ANALOG_GAIN_SET_VALUE;

	cis_data->max_analog_gain[0] = read_value;

	cis_data->max_analog_gain[1] = sensor_gc02m2_cis_calc_again_permile(read_value);
...
int sensor_gc02m2_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_gc02m2_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->min_digital_gain[0] = SENSOR_GC02M2_MIN_DIGITAL_GAIN_SET_VALUE;
...
int sensor_gc02m2_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->max_digital_gain[0] = SENSOR_GC02M2_MAX_DIGITAL_GAIN_SET_VALUE;
...
int sensor_gc02m2_cis_set_totalgain(struct v4l2_subdev *subdev, struct ae_param *target_exposure,
	struct ae_param *again, struct ae_param *dgain)
{
	...
	/* Set Exposure Time */
	ret = sensor_gc02m2_cis_set_exposure_time(subdev, multiple_exp);
	if (ret < 0) {
		err("[%s] sensor_gc02m2_cis_set_exposure_time fail\n", __func__);
		goto p_err;
	}
	...
	ret = sensor_gc02m2_cis_set_analog_gain(subdev, input_again);
	if (ret < 0) {
		err("[%s] sensor_gc02m2_cis_set_analog_gain fail\n", __func__);
		goto p_err;
	}
...
int sensor_gc02m2_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct is_cis *cis;
...
int sensor_gc02m2_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct is_cis *cis;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_gc02m2_cis_init,
	.cis_log_status = sensor_gc02m2_cis_log_status,
	.cis_set_global_setting = sensor_gc02m2_cis_set_global_setting,
	.cis_mode_change = sensor_gc02m2_cis_mode_change,
	.cis_set_size = sensor_gc02m2_cis_set_size,
	.cis_stream_on = sensor_gc02m2_cis_stream_on,
	.cis_stream_off = sensor_gc02m2_cis_stream_off,
	.cis_set_exposure_time = NULL,
	.cis_get_min_exposure_time = sensor_gc02m2_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_gc02m2_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_gc02m2_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_gc02m2_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_gc02m2_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_gc02m2_cis_adjust_analog_gain,
	.cis_set_analog_gain = NULL,
	.cis_get_analog_gain = sensor_gc02m2_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_gc02m2_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_gc02m2_cis_get_max_analog_gain,
	.cis_set_digital_gain = NULL,
	.cis_get_digital_gain = sensor_gc02m2_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_gc02m2_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_gc02m2_cis_get_max_digital_gain,
	.cis_compensate_gain_for_extremely_br = sensor_cis_compensate_gain_for_extremely_br,
	.cis_wait_streamoff = sensor_gc02m2_cis_wait_streamoff,
	.cis_wait_streamon = sensor_gc02m2_cis_wait_streamon,
	.cis_check_rev_on_init = sensor_gc02m2_cis_check_rev,
	.cis_set_totalgain = sensor_gc02m2_cis_set_totalgain,
};

int cis_gc02m2_probe(struct i2c_client *client,
...
//#endif
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		sensor_gc02m2_global = sensor_gc02m2_setfile_A_Global;
		sensor_gc02m2_global_size = ARRAY_SIZE(sensor_gc02m2_setfile_A_Global);
		sensor_gc02m2_setfiles = sensor_gc02m2_setfiles_A;
		sensor_gc02m2_setfile_sizes = sensor_gc02m2_setfile_A_sizes;
		sensor_gc02m2_pllinfos = sensor_gc02m2_pllinfos_A;
		sensor_gc02m2_max_setfile_num = ARRAY_SIZE(sensor_gc02m2_setfiles_A);
		sensor_gc02m2_fsync_master = sensor_gc02m2_setfile_A_Fsync_Master;
		sensor_gc02m2_fsync_master_size = ARRAY_SIZE(sensor_gc02m2_setfile_A_Fsync_Master);
		sensor_gc02m2_fsync_slave = sensor_gc02m2_setfile_A_Fsync_Slave;
		sensor_gc02m2_fsync_slave_size = ARRAY_SIZE(sensor_gc02m2_setfile_A_Fsync_Slave);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_gc02m2_global = sensor_gc02m2_setfile_B_Global;
		sensor_gc02m2_global_size = ARRAY_SIZE(sensor_gc02m2_setfile_B_Global);
		sensor_gc02m2_setfiles = sensor_gc02m2_setfiles_B;
		sensor_gc02m2_setfile_sizes = sensor_gc02m2_setfile_B_sizes;
		sensor_gc02m2_pllinfos = sensor_gc02m2_pllinfos_B;
		sensor_gc02m2_max_setfile_num = ARRAY_SIZE(sensor_gc02m2_setfiles_B);
		sensor_gc02m2_fsync_master = sensor_gc02m2_setfile_B_Fsync_Master;
		sensor_gc02m2_fsync_master_size = ARRAY_SIZE(sensor_gc02m2_setfile_B_Fsync_Master);
		sensor_gc02m2_fsync_slave = sensor_gc02m2_setfile_B_Fsync_Slave;
		sensor_gc02m2_fsync_slave_size = ARRAY_SIZE(sensor_gc02m2_setfile_B_Fsync_Slave);
	} else {
...
static const struct of_device_id sensor_cis_gc02m2_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-gc02m2",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_gc02m2_match);

static const struct i2c_device_id sensor_cis_gc02m2_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_gc02m2_driver = {
	.probe	= cis_gc02m2_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_gc02m2_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_gc02m2_idt,
};

static int __init sensor_cis_gc02m2_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_gc02m2_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_gc02m2_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_gc02m2_init);
MODULE_LICENSE("GPL");
