...
int sensor_imx374_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_log_status(cis, client, log_imx374,
...
int sensor_imx374_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	/* HDR OFF */
	ret |= is_sensor_write8(cis->client, 0x0220, 0x00);
...
static int sensor_imx374_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8(client, 0xA02, 0x5F);
	if (ret < 0) {
		ret = -EAGAIN;
		goto p_err;
	}
...
static int cis_imx374_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
		...
		cis->mipi_sensor_mode = sensor_imx374_setfile_A_19p2_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_imx374_setfile_A_19p2_mipi_sensor_mode);
