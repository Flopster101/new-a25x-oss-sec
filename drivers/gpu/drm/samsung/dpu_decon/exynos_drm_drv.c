...
int decon_no_display;
module_param(decon_no_display, int, S_IRUGO);
EXPORT_SYMBOL(decon_no_display);
int decon_bypass_display;
EXPORT_SYMBOL(decon_bypass_display);
int decon_commit_retry;
EXPORT_SYMBOL(decon_commit_retry);
#endif
...
static int __init exynos_drm_init(void)
{
	...
	if (sec_needs_decon) {
		printk(KERN_INFO "%s Initialized Exynos DPU driver for decon\n", sec_detect_label);
