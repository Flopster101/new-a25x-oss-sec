...
void dp_logger_print_date_time(void)
{
	...
	decon_dp_logger_print("%s\n", tmp);
}

/* set max log count, if count is -1, no limit */
void decon_dp_logger_set_max_count(int count)
{
	log_max_count = count;
...
EXPORT_SYMBOL(decon_dp_logger_set_max_count);

void decon_dp_logger_print(const char *fmt, ...)
{
	int len;
	va_list args;
...
EXPORT_SYMBOL(decon_dp_logger_print);

void decon_dp_logger_hex_dump(void *buf, void *pref, size_t size)
{
	uint8_t *ptr = buf;
	size_t i;
	char tmp[128] = {0x0, };
...
EXPORT_SYMBOL(decon_dp_logger_hex_dump);
...
int decon_dp_logger_init(void)
{
	...
	is_dp_logger_init = 1;
	decon_dp_logger_print("dp logger init ok\n");

	return 0;
}
EXPORT_SYMBOL(decon_dp_logger_init);
