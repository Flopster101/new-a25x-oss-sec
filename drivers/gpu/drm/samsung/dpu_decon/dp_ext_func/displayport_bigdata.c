...
void decon_secdp_bigdata_init(struct class *dp_class)
{
	int ret;
...
EXPORT_SYMBOL(decon_secdp_bigdata_init);
...
void decon_secdp_bigdata_save_item(enum DP_BD_ITEM_LIST item, ...)
{
	va_list  vl;
...
EXPORT_SYMBOL(decon_secdp_bigdata_save_item);

void decon_secdp_bigdata_inc_error_cnt(enum DP_BD_ITEM_LIST err)
{
...
EXPORT_SYMBOL(decon_secdp_bigdata_inc_error_cnt);

void decon_secdp_bigdata_clr_error_cnt(enum DP_BD_ITEM_LIST err)
{
	if (err >= BD_ITEM_MAX || err < 0)
		return;
...
EXPORT_SYMBOL(decon_secdp_bigdata_clr_error_cnt);
...
void decon_secdp_bigdata_connection(void)
{
	int i;
	...
	for (i = 0; i < BD_ITEM_MAX; i++) {
		...
		case CHR:
			...
			break;
		case ERR:
			decon_secdp_bigdata_clr_error_cnt(i);
			break;
		default:
			break;
		}
...
EXPORT_SYMBOL(decon_secdp_bigdata_connection);

void decon_secdp_bigdata_disconnection(void)
{
...
EXPORT_SYMBOL(decon_secdp_bigdata_disconnection);
