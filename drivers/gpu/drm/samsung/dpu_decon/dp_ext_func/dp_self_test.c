...
enum dex_support_type decon_self_test_get_dp_adapter_type(void)
{
	return g_test_vars.adapter_type;
}
EXPORT_SYMBOL(decon_self_test_get_dp_adapter_type);

int decon_self_test_on_process(void)
{
	return g_test_vars.test_on_process;
}
EXPORT_SYMBOL(decon_self_test_on_process);

int decon_self_test_get_edid(u8 *edid)
{
	memcpy(edid, test_data[g_test_vars.data_idx].edid, 256);
	return edid[0x7e] + 1;
}
EXPORT_SYMBOL(decon_self_test_get_edid);

void decon_self_test_resolution_update(u32 xres, u32 yres, u32 fps)
{
...
EXPORT_SYMBOL(decon_self_test_resolution_update);
...
void decon_self_test_init(struct dp_device *dp, struct class *dp_class)
{
	int ret = 0;
...
EXPORT_SYMBOL(decon_self_test_init);
