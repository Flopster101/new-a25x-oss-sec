...
int decon_exynos_dpuf_set_votf(u32 dpuf_idx, bool en) { return 0; }
EXPORT_SYMBOL(decon_exynos_dpuf_set_votf);
...
struct dpp_device *decon_of_find_dpp_by_node(struct device_node *np)
{
	struct device *dev;
...
EXPORT_SYMBOL(decon_of_find_dpp_by_node);
#endif
