...
bool mcd_drm_check_commit_skip(struct exynos_drm_crtc *exynos_crtc, const char *caller)
{
	...
	if (decon_no_display || decon_bypass_display) {
		pr_info("%s: decon_no_display: %d, decon_bypass_display: %d\n", caller, decon_no_display, decon_bypass_display);
		ret = true;
	}
...
bool mcd_drm_check_commit_retry(struct exynos_drm_crtc *exynos_crtc, const char *caller)
{
	...
	if (decon_commit_retry) {
		pr_info("%s: decon_commit_retry: %d\n", caller, decon_commit_retry);
		ret = true;
	}
