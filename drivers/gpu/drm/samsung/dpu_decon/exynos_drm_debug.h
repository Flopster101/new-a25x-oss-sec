...
extern struct memlog_obj *decon_g_log_obj;
extern struct memlog_obj *decon_g_errlog_obj;
	...
	do {									\
		if ((log_lv) >= 3)						\
			pr_err(DPU_PR_PREFIX DPU_PR_FMT fmt,			\
				DPU_PR_ARG((name), (id)), ##__VA_ARGS__);	\
										\
		dpu_pr_memlog((decon_g_log_obj), (name), (id), MEMLOG_LEVEL_ERR,	\
				fmt, ##__VA_ARGS__);				\
		dpu_pr_memlog((decon_g_errlog_obj), (name), (id), MEMLOG_LEVEL_ERR,	\
				fmt, ##__VA_ARGS__);				\
	} while (0)
	...
	do {									\
		if ((log_lv) >= 5)						\
			pr_warn(DPU_PR_PREFIX DPU_PR_FMT fmt,			\
				DPU_PR_ARG((name), (id)), ##__VA_ARGS__);	\
										\
		dpu_pr_memlog(decon_g_log_obj, name, id, MEMLOG_LEVEL_CAUTION,	\
				fmt, ##__VA_ARGS__);				\
	} while (0)
	...
	do {									\
		if ((log_lv) >= 6)						\
			pr_info(DPU_PR_PREFIX DPU_PR_FMT fmt,			\
				DPU_PR_ARG((name), (id)), ##__VA_ARGS__);	\
										\
		dpu_pr_memlog((decon_g_log_obj), (name), (id), MEMLOG_LEVEL_INFO,	\
				fmt, ##__VA_ARGS__);				\
	} while (0)
	...
	do {									\
		if ((log_lv) >= 7)						\
			pr_debug(DPU_PR_PREFIX DPU_PR_FMT fmt,			\
				DPU_PR_ARG((name), (id)), ##__VA_ARGS__);	\
										\
		dpu_pr_memlog((decon_g_log_obj), (name), (id), MEMLOG_LEVEL_DEBUG,	\
				fmt, ##__VA_ARGS__);				\
	} while (0)
