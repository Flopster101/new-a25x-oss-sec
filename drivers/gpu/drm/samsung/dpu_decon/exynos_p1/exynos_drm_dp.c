...
struct dp_device *decon_dp_drvdata;
EXPORT_SYMBOL(decon_dp_drvdata);
...
struct blocking_notifier_head decon_dp_ado_notifier_head =
		BLOCKING_NOTIFIER_INIT(decon_dp_ado_notifier_head);
EXPORT_SYMBOL(decon_dp_ado_notifier_head);
#endif

void decon_dp_hdcp22_enable(u32 en);
...
static int dp_full_link_training(struct dp_device *dp)
{
	...
	decon_secdp_bigdata_save_item(BD_MAX_LANE_COUNT, lane_cnt);
	decon_secdp_bigdata_save_item(BD_MAX_LINK_RATE, link_rate);
#endif
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
	decon_secdp_bigdata_clr_error_cnt(ERR_LINK_TRAIN);
	decon_secdp_bigdata_save_item(BD_CUR_LANE_COUNT, lane_cnt);
	decon_secdp_bigdata_save_item(BD_CUR_LINK_RATE, link_rate);
#endif
#ifdef FEATURE_USE_DRM_EDID_PARSER
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
	decon_secdp_bigdata_save_item(BD_ADAPTER_TYPE, dfp[port_type]);
#endif
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		decon_secdp_bigdata_save_item(BD_ADAPTER_HWID, val[0]);
		decon_secdp_bigdata_save_item(BD_ADAPTER_FWVER, (val[1] << 8) | val[2]);
#endif
	}
...
static int dp_link_training(struct dp_device *dp)
{
	...
	if (ret < 0) {
		...
		//dp_debug_dump();
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
			decon_secdp_bigdata_inc_error_cnt(ERR_EDID);
#endif
	}
...
#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_DISPLAYPORT)
		blocking_notifier_call_chain(&decon_dp_ado_notifier_head,
		...
		enum hotplug_state state)
{
	...
	decon_dp_logger_set_max_count(500);
#endif
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
			decon_secdp_bigdata_inc_error_cnt(ERR_INF_IRQHPD);
#endif
#if IS_ENABLED(CONFIG_EXYNOS_HDCP2)
			hdcp_dplink_set_reauth();
#endif
			decon_dp_hdcp22_enable(0);
...
#endif
int decon_dp_audio_config(struct dp_audio_config_data *audio_config_data)
{
	struct dp_device *dp = get_dp_drvdata();
	int ret = 0;
	...
	if (audio_config_data->audio_enable == AUDIO_ENABLE) {
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		{
			...
			decon_secdp_bigdata_save_item(BD_AUD_CH, audio_config_data->audio_channel_cnt);
			if (audio_config_data->audio_fs >= 0 && audio_config_data->audio_fs < 7)
				decon_secdp_bigdata_save_item(BD_AUD_FREQ, fs[audio_config_data->audio_fs]);
			if (audio_config_data->audio_bit >= 0 && audio_config_data->audio_bit < 3)
				decon_secdp_bigdata_save_item(BD_AUD_BIT, bit[audio_config_data->audio_bit]);
		}
#endif
...
EXPORT_SYMBOL(decon_dp_audio_config);
...
int decon_dp_dpcd_read_for_hdcp22(u32 address, u32 length, u8 *data)
{
	struct dp_device *dp = get_dp_drvdata();
	int ret;
...
int decon_dp_dpcd_write_for_hdcp22(u32 address, u32 length, u8 *data)
{
	struct dp_device *dp = get_dp_drvdata();
	int ret;
...
EXPORT_SYMBOL(decon_dp_hdcp22_enable);
EXPORT_SYMBOL(decon_dp_dpcd_write_for_hdcp22);
EXPORT_SYMBOL(decon_dp_dpcd_read_for_hdcp22);
...
static void dp_hdcp22_run(struct work_struct *work)
{
	...
	if (ret) {
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		decon_secdp_bigdata_inc_error_cnt(ERR_HDCP_AUTH);
#endif
		goto exit_hdcp;
	}
	...
	decon_dp_dpcd_read_for_hdcp22(DPCD_HDCP22_RX_INFO, 2, val);
...
static int dp_check_hdcp_version(struct dp_device *dp)
{
...
#endif
	decon_dp_dpcd_read_for_hdcp22(DPCD_HDCP22_RX_CAPS, DPCD_HDCP22_RX_CAPS_LENGTH, val);
	...
	if (ret == HDCP_VERSION_2_2)
		decon_secdp_bigdata_save_item(BD_HDCP_VER, "hdcp2");
	else if (ret == HDCP_VERSION_1_3)
		decon_secdp_bigdata_save_item(BD_HDCP_VER, "hdcp1");
#endif
...
#ifdef CONFIG_SEC_DISPLAYPORT_SELFTEST
	if (decon_self_test_on_process()) {
		decon_self_test_resolution_update(dp->cur_mode.hdisplay,
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		decon_secdp_bigdata_save_item(BD_ORIENTATION,	dp->cal_res.dp_sw_sel ? "CC1" : "CC2");
#endif
				...
				PD_NOTI_TYPEDEF *usb_typec_info)
{
	...
	case PDIC_NOTIFY_ID_DP_CONNECT:
		...
		case PDIC_NOTIFY_DETACH:
#ifdef CONFIG_SEC_DISPLAYPORT_LOGGER
			decon_dp_logger_set_max_count(100);
#endif
			...
			dp_aux_onoff(dp, 0);
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
			decon_secdp_bigdata_disconnection();
#endif
			break;
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
			decon_secdp_bigdata_connection();
			decon_secdp_bigdata_save_item(BD_ADT_VID, dp->sink_info.ven_id);
			decon_secdp_bigdata_save_item(BD_ADT_PID, dp->sink_info.prod_id);
#endif
			break;
		default:
			break;
	...
	case PDIC_NOTIFY_ID_DP_LINK_CONF:
		...
		dp_aux_sel(dp);
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		decon_secdp_bigdata_save_item(BD_LINK_CONFIGURE, usb_typec_info->sub1 + 'A' - 1);
#endif
		switch (usb_typec_info->sub1) {
...
static int dp_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
		decon_secdp_bigdata_init(dp_class);
#endif
#ifdef CONFIG_SEC_DISPLAYPORT_SELFTEST
		dp->hpd_changed = dp_hpd_changed;
		decon_self_test_init(dp, dp_class);
#endif
	}
	...
	dp_register_func_for_hdcp22(decon_dp_hdcp22_enable,
			decon_dp_dpcd_read_for_hdcp22,
			decon_dp_dpcd_write_for_hdcp22);
#endif
