		...
		int block, u8 *buf, size_t len)
{
	...
	decon_dp_logger_hex_dump(buf, "EDID: ", 128);
#endif
...
static void dp_get_dex_adapter_type(struct dp_device *dp)
{
	...
	if (decon_self_test_on_process()) {
		dp->dex.adapter_type = decon_self_test_get_dp_adapter_type();
		return;
	}
#endif
...
#ifdef CONFIG_SEC_DISPLAYPORT_SELFTEST
	if (decon_self_test_on_process()) {
		ret = decon_self_test_get_edid(dp->rx_edid_data.edid_buf);
...
u8 edid_read_checksum(struct dp_device *dp)
{
	...
	if (ret)
		return ret;
#ifdef CONFIG_SEC_DISPLAYPORT_LOGGER
	decon_dp_logger_hex_dump(buf, "Test EDID: ", 128);
#endif
	block = buf[0x7e];
	for (i = 1; i <= block; i++) {
