...
dpu_pr_info(drv_name((panel)), 0, decon_get_panel_log_level(), fmt, ##__VA_ARGS__)

#define panel_warn(panel, fmt, ...)	\
dpu_pr_warn(drv_name((panel)), 0, decon_get_panel_log_level(), fmt, ##__VA_ARGS__)

#define panel_err(panel, fmt, ...)	\
dpu_pr_err(drv_name((panel)), 0, decon_get_panel_log_level(), fmt, ##__VA_ARGS__)

#define panel_debug(panel, fmt, ...)	\
dpu_pr_debug(drv_name((panel)), 0, decon_get_panel_log_level(), fmt, ##__VA_ARGS__)
...
extern int decon_bypass_display;
extern int panel_cmd_log_level;
extern int decon_commit_retry;
#endif
...
int decon_dsim_host_cmdset_transfer(struct mipi_dsi_host *host,
			      struct mipi_dsi_msg *msg, int cmd_cnt,
			      bool wait_vsync, bool wait_fifo);
int decon_exynos_drm_cmdset_add(struct exynos_panel *ctx, u8 type, size_t size, const u8 *data);
