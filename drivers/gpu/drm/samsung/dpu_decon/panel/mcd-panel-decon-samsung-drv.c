...
int decon_get_panel_log_level(void)
{
	return panel_log_level;
}
EXPORT_SYMBOL(decon_get_panel_log_level);
...
static inline bool is_panel_tui(struct exynos_panel *ctx)
{
	...
	if (conn_state && conn_state->crtc)
		return decon_is_tui_trans(conn_state->crtc->state);

	return false;
}

int decon_mcd_drm_panel_get_modes(struct drm_panel *panel, struct drm_connector *conn)
{
...
EXPORT_SYMBOL(decon_mcd_drm_panel_get_modes);

void decon_exynos_panel_set_lp_mode(struct exynos_panel *ctx, const struct exynos_panel_mode *pmode)
{
	int ret;
...
EXPORT_SYMBOL(decon_exynos_panel_set_lp_mode);
...
int decon_exynos_drm_cmdset_add(struct exynos_panel *ctx, u8 type, size_t size,
			  const u8 *data)
{
	u8 *buf;
	int index;
...
EXPORT_SYMBOL(decon_exynos_drm_cmdset_add);

int decon_exynos_drm_cmdset_cleanup(struct exynos_panel *ctx)
{
	int i;
...
EXPORT_SYMBOL(decon_exynos_drm_cmdset_cleanup);

int decon_exynos_drm_cmdset_flush(struct exynos_panel *ctx, bool wait_vsync, bool wait_fifo)
{
	int ret;
	...
	ret = decon_dsim_host_cmdset_transfer(dsi->host, ctx->msg, ctx->cmdset_msg_total, wait_vsync, wait_fifo);
	if (ret < 0)
		panel_err(ctx, "failed to tx command set data\n");

	ret = decon_exynos_drm_cmdset_cleanup(ctx);
	if (ret < 0)
		panel_err(ctx, "failed to cleanup command set data\n");

	return ret;
}
EXPORT_SYMBOL(decon_exynos_drm_cmdset_flush);
				       ...
				       const struct exynos_panel_desc *desc)
{
	struct exynos_drm_connector_properties *p =
		decon_exynos_drm_connector_get_properties(exynos_conn);
					...
					enum drm_bridge_attach_flags flags)
{
	...
	ret = decon_exynos_drm_connector_init(bridge->dev, &ctx->exynos_connector,
...
static void exynos_panel_enable(struct drm_bridge *bridge)
{
	...
	if (ctx->desc->lp11_reset) {
		...
		/* 2. LP11 -> HS CLK (dsim) */
		decon_dsim_atomic_activate(bridge->encoder);
	}
...
#else
		return decon_mcd_decon_get_bts_fps(&pmode->mode);
#endif
	}
		...
		const struct exynos_panel_mode *pmode)
{
	...
	pdm = decon_exynos_panel_find_panel_mode(pdms, mode);
	if (pdm == NULL) {
...
int decon_drm_mipi_fcmd_write(void *_ctx, const u8 *payload, int size, u32 align)
{
	struct exynos_panel *ctx = (struct exynos_panel *)_ctx;
	struct mipi_dsi_device *dsi;
	struct dsim_device *dsim;
	...
	ret = decon_dsim_host_fcmd_transfer(dsi->host, &fcmd->msg);
	if (ret < 0)
...
EXPORT_SYMBOL(decon_drm_mipi_fcmd_write);
...
{
	return decon_drm_mipi_fcmd_write((struct exynos_panel *)_ctx, payload, size,
...
//	bool wait_tx_done = false;
	...
	/* 1. Cleanup Buffer */
	decon_exynos_drm_cmdset_cleanup(ctx);
	...
	for (i = 0; i < size; i++) {
		...
		if ((i - from >= (MAX_CMDSET_NUM - 1)) ||
			...
			if (decon_exynos_drm_cmdset_flush(ctx, wait_vsync, wait_tx_done)) {
				panel_err(ctx, "failed to decon_exynos_drm_cmdset_flush\n");
				ret = -EIO;
				goto error;
			}
		...
		if (cmd[i].cmd_id == MIPI_DSI_WR_DSC_CMD) {
			ret = decon_exynos_drm_cmdset_add(ctx, MIPI_DSI_COMPRESSION_MODE, cmd[i].size, cmd[i].buf);
		} else if (cmd[i].cmd_id == MIPI_DSI_WR_PPS_CMD) {
			ret = decon_exynos_drm_cmdset_add(ctx, MIPI_DSI_PICTURE_PARAMETER_SET, cmd[i].size, cmd[i].buf);
		} else if (cmd[i].cmd_id == MIPI_DSI_WR_GEN_CMD) {
			if (cmd[i].size == 1)
				ret = decon_exynos_drm_cmdset_add(ctx, MIPI_DSI_DCS_SHORT_WRITE, cmd[i].size, cmd[i].buf);
			else
				ret = decon_exynos_drm_cmdset_add(ctx, MIPI_DSI_DCS_LONG_WRITE, cmd[i].size, cmd[i].buf);
		} else {
		...
		if (ret) {
			panel_err(ctx, "failed to decon_exynos_drm_cmdset_add\n");
			goto error;
		}
...
{
	pr_info("%s %s\n", __func__, on ? "on" : "off");
	decon_commit_retry = (on ? 1 : 0);
	return 0;
}

__visible_for_testing int mcd_drm_get_bypass(void *ctx)
{
	return (decon_bypass_display > 0 ? 1 : 0);
}
		...
		struct freq_hop_param *param)
{
	...
	dsim = container_of(dsi->host, struct dsim_device, dsi_host);
	ret = decon_mcd_dsim_update_dsi_freq(dsim, param->dsi_freq);
	if (ret < 0) {
...
static const struct drm_panel_funcs mcd_drm_panel_funcs = {
	...
	.get_modes = decon_mcd_drm_panel_get_modes,
};

static const struct exynos_panel_funcs mcd_exynos_panel_funcs = {
	.set_lp_mode = decon_exynos_panel_set_lp_mode,
...
mcd_drm_panel_get_exynos_panel_desc(struct exynos_panel *ctx)
{
	...
	desc = decon_exynos_panel_desc_create_from_panel_display_modes(ctx, pdms);
...
int decon_exynos_panel_probe(struct mipi_dsi_device *dsi)
{
...
EXPORT_SYMBOL(decon_exynos_panel_probe);

int decon_exynos_panel_remove(struct mipi_dsi_device *dsi)
{
	...
	decon_exynos_panel_desc_destroy(ctx, (struct exynos_panel_desc *)ctx->desc);

	return 0;
}
EXPORT_SYMBOL(decon_exynos_panel_remove);
...
static struct mipi_dsi_driver exynos_panel_driver = {
	.probe = decon_exynos_panel_probe,
	.remove = decon_exynos_panel_remove,
	.driver = {
...
MODULE_SOFTDEP("pre: mcd-panel-decon");
