...
struct memlog_obj *decon_g_log_obj;
EXPORT_SYMBOL(decon_g_log_obj);

struct memlog_obj *decon_g_errlog_obj;
EXPORT_SYMBOL(decon_g_errlog_obj);
...
static void dpu_init_memlogger(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (!decon_g_log_obj) {
		decon_g_log_obj = memlog_alloc_printf(memlog->desc, DPU_MEMLOG_SIZE,
				NULL, "log-mem0", 0);
		if (!decon_g_log_obj) {
	...
	if (!decon_g_errlog_obj) {
		decon_g_errlog_obj = memlog_alloc_printf(memlog->desc, DPU_ERRMEMLOG_SIZE,
				NULL, "log-mem1", 0);
		if (!decon_g_errlog_obj) {
