...
void decon_exynos_drm_boost_bts_fps(struct exynos_drm_connector *exynos_connector,
		u32 fps, ktime_t expire_time)
{
	exynos_connector->boost_bts_fps = fps;
	exynos_connector->boost_expire_time = expire_time;
}
EXPORT_SYMBOL(decon_exynos_drm_boost_bts_fps);

struct exynos_drm_connector_properties *
decon_exynos_drm_connector_get_properties(struct exynos_drm_connector *exynos_connector)
{
	return dev_get_exynos_properties(exynos_connector->base.dev,
			DRM_MODE_OBJECT_CONNECTOR);
}
EXPORT_SYMBOL(decon_exynos_drm_connector_get_properties);
...
int decon_exynos_drm_connector_init(struct drm_device *dev,
...
EXPORT_SYMBOL(decon_exynos_drm_connector_init);
