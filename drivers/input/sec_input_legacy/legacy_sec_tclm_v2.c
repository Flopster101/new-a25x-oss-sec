...
void legacy_sec_tclm_case(struct sec_tclm_data *data, int tclm_case)
{
	...
	case 'f':
		if (data->external_factory == true)
			legacy_sec_tclm_root_of_cal(data, CALPOSITION_OUTSIDE);
		else
			legacy_sec_tclm_root_of_cal(data, CALPOSITION_FACTORY);
		break;

	case 'L':
	case 'l':
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_LCIA);
		break;

	case 'C':
	case 'c':
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_SVCCENTER);
		break;
	...
	default:
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_ABNORMAL);
	}
}
EXPORT_SYMBOL(legacy_sec_tclm_case);
...
int legacy_tclm_test_command(struct sec_tclm_data *data, int test_case, int cmd_param1, int cmd_param2, char *buff)
{
	...
	case 1: /* change cal_position & history
		...
		if (cmd_param1 == 0xff)
			legacy_sec_tclm_root_of_cal(data, 0);
		else
			legacy_sec_tclm_root_of_cal(data, cmd_param1);
		...
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_NONE);
...
EXPORT_SYMBOL(legacy_tclm_test_command);
...
EXPORT_SYMBOL(legacy_sec_tclm_test_on_probe);
...
EXPORT_SYMBOL(legacy_sec_tclm_debug_info);
...
EXPORT_SYMBOL(legacy_sec_tclm_root_of_cal);
...
EXPORT_SYMBOL(legacy_sec_execute_tclm_package);
...
int legacy_sec_tclm_check_cal_case(struct sec_tclm_data *data)
{
	...
	if ((data->nvdata.cal_count == 0) || (data->nvdata.cal_count == 0xFF)) {
		...
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_INITIAL);
		restore_cal = 1;
	} else if (data->tclm_level == TCLM_LEVEL_EVERYTIME) {
		/* everytime case */
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_EVERYTIME);
		restore_cal = 1;
	}

	if (restore_cal) {
		ret = legacy_sec_execute_tclm_package(data, 0);
		if (ret < 0) {
			input_err(true, dev, "%s: fail legacy_sec_execute_tclm_package!\n", __func__);
			return ret;
		}
		legacy_sec_tclm_root_of_cal(data, CALPOSITION_NONE);
		return ret;
...
EXPORT_SYMBOL(legacy_sec_tclm_check_cal_case);
...
EXPORT_SYMBOL(legacy_sec_tclm_initialize);
